...
func evaluationCases(t *testing.T) (*Env, map[string]string) {
	...
	cases := map[string]string{
		...
		"string.Trim":                        `@("  a  ".Trim())`,
		"string.TrimStart":                   `@("  a".TrimStart())`,
		"string.TrimEnd":                     `@("a  ".TrimEnd())`,
		"string.ToLower":                     `@("A".ToLower())`,
		"string.ToUpper":                     `@("a".ToUpper())`,
		"string.StartsWith":                  `@("ab".StartsWith("a"))`,
		"string.EndsWith":                    `@("ab".EndsWith("b"))`,
		"string.Contains":                    `@("ab".Contains("a"))`,
		"string.Equals":                      `@("ab".Equals("ab"))`,
		"string.Replace":                     `@("ab".Replace("a", "c"))`,
		"string.Substring":                   `@("abc".Substring(1))`,
		"string.IndexOf":                     `@("abc".IndexOf("b"))`,
		"string.GetHashCode":                 `@("abc".GetHashCode())`,
...
func TestAllowlistDoesNotInventContextMembers(t *testing.T) {
	...
	helpers := map[string]bool{
		...
		"Variables": true, "value": true, "Certificates": true,
	}
	for _, member := range Allowlist() {
