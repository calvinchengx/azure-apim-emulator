...
- Operator portal for resources, snapshots, traces, clock, faults, and parity.
  The embedded operator portal now exposes runtime status, active snapshot, parity status, trace navigation, controllable clock state, and persisted service/API summaries under `/_emulator/portal/`; fault controls and resource mutation editors remain open.
