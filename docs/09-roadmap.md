...
- Core policy families: routing, mutation, control flow, auth, limits, cache, validation, send-request, transforms, and tracing.
  Literal and expression-evaluated query/body/variable/method/header mutations, CORS response/preflight handling, `check-header`, `validate-jwt` backed by the configured Entra validator, `ip-filter` validation, injected `send-request` calls, deterministic `rate-limit`/`quota` and keyed windows with nested `api`/`operation` children and quota `bandwidth`, keyed `limit-concurrency` slots, first-class sequential `wait` under `limit-concurrency` and `retry`, response and value cache lookup/store/removal with gateway TTL integration, `validate-status-code`, `validate-content`, `validate-headers`, `validate-parameters`, `validate-client-certificate`, `choose` control flow, `trace` events, `authentication-basic`, `authentication-managed-identity`, `authentication-oauth2`, `authentication-certificate`, `find-and-replace`, `json-to-xml`, `xml-to-json`, `jsonp` transforms, standalone `set-status`, empty-body `mock-response`, compile-time `include-fragment` expansion, fire-and-forget `send-one-way-request`, configured-validator `validate-azure-ad-token`, Adobe `cross-domain` XML, and gateway/backend `redirect-content-urls` are implemented; example/schema mock bodies, Entra claim/app-id constraints, and the remaining families are still open.
