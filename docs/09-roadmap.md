...
- APIs, revisions, releases, versions, operations, schemas, tags, products,
  ...
  APIs, operations, products, groups, API version sets, tags, and policy fragments now have canonical lossless documents; recursive PATCH is implemented where the stable surface exposes PATCH, and the same persistence pattern remains to be extended across other core resources.
