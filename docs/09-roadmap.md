...
- Complete ARM common semantics: ETags, paging, filters, patch, LROs, errors, and secret operations.
  Conditional ETag evaluation, stable-contract required `If-Match` enforcement for implemented entity mutations, common collection paging/filter grammar, undocumented OData-option rejection, policy-fragment `$orderby=name`, and the stable scalar field/operator/function matrix for every implemented collection shape are implemented centrally; named-value `any`/`all` tag predicates, named collection selectors, and the remaining semantics are still open.
