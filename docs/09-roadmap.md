...
- Operator portal for resources, snapshots, traces, clock, faults, and parity.
  The embedded operator portal now exposes runtime status, active snapshot, parity status, trace navigation, controllable clock state, persisted service/API summaries, policy read/update editing, and deterministic backend fault controls under `/_emulator/portal/`; service-to-API fallback and API `<base/>` composition are implemented, while broader scope inheritance and resource editors remain open.
