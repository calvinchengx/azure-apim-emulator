...
- HTTP routing edge cases, custom domains, backend TLS, pools, retries, circuit breakers, streaming, SSE, and WebSocket.
  Custom hostname routing from persisted `hostnameConfigurations` is implemented; backend TLS policy, retries, circuit breakers, streaming, SSE, and WebSocket remain open.
