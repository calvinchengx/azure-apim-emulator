...
- APIs, revisions, releases, versions, operations, schemas, tags, products,
  ...
  APIs, operations, schemas, releases, products, groups, users, subscriptions, named values, backends, certificates, API version sets, tags, and policy fragments now have canonical lossless documents; user/subscription/named-value/certificate documents enforce store-level secret redaction, recursive PATCH is implemented where the stable surface exposes PATCH, and the same persistence pattern remains to be extended across other core resources.
