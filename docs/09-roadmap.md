...
- Core policy families: routing, mutation, control flow, auth, limits, cache, validation, send-request, transforms, and tracing.
  Literal query/body/variable mutations, `check-header`, `validate-jwt` backed by the configured Entra validator, and `ip-filter` validation are implemented; the remaining families are still open.
