...
- Core policy families: routing, mutation, control flow, auth, limits, cache, validation, send-request, transforms, and tracing.
  Literal query/body/variable/method mutations, CORS response/preflight handling, `check-header`, `validate-jwt` backed by the configured Entra validator, `ip-filter` validation, injected `send-request` calls, deterministic `rate-limit-by-key`/`quota-by-key` windows, and TTL cache lookup/store are implemented; the remaining families are still open.
