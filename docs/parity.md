...
| APIs/operations/schemas/tags/products/groups/subscriptions/named values/backends/certificates | partial | P0-P1 | Core lifecycles and four-SDK secret rotation; Go SDK revision cloning including lossless schemas and API/operation tag associations, tag CRUD plus API/operation/product links, built-in/custom/external groups and product-group links, release promotion, version-set, named-value, backend, and PFX certificate CRUD/refresh; policy-substituted, backend-ID, and real backend mTLS gateway workflows; Key Vault retrieval and broader resources pending |
