...
| APIs/operations/schemas/products/subscriptions/named values/backends/certificates | partial | P0-P1 | Core lifecycles and four-SDK secret rotation; Go SDK revision cloning including lossless schemas, release promotion, version-set, named-value, backend, and PFX certificate CRUD/refresh; policy-substituted, backend-ID, and real backend mTLS gateway workflows; Key Vault retrieval and broader resources pending |
