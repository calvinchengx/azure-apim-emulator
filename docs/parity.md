...
| APIs/operations/products/subscriptions | partial | P0-P1 | API, operation, product, product/API-link, and subscription lifecycles; secret rotation passes all four SDKs; revision metadata/listing and atomic source cloning of operations/policies pass the Go SDK, with non-current revisions excluded from public routes; releases, versions, and broader schemas pending |
