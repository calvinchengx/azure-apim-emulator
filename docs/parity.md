...
| APIs/operations/schemas/tags/products/groups/users/subscriptions/named values/backends/certificates | partial | P0-P1 | Core lifecycles and four-SDK secret rotation; Go SDK revision cloning including lossless schemas and API/operation tag associations, tag CRUD plus API/operation/product links, groups and product links, user CRUD/SSO/tokens and bidirectional group membership, release promotion, version-set, named-value, backend, and PFX certificate CRUD/refresh; policy-substituted, backend-ID, and real backend mTLS gateway workflows; Key Vault retrieval and broader resources pending |
