...
The exception is real: `scripts/check_docs_links.py` fails on a link that
points at a page which does not exist, and `check_witnesses.py` fails on a
green parity row whose witness is gone. Both read `docs/`, so both stay
ungated and run on every change. A docs change can still break them — but only
by REMOVING or RENAMING a page, never by adding or editing one, so a delete or
a rename anywhere in the change disqualifies the cheap lane whatever the paths
say.
...
def self_test() -> int:
    ...
    cases: list[tuple[str, bool, str]] = [
        ...
        ("M\te2e/openid/witness.js", False, "a witness is code"),
        ("M\tsite/index.html", True, "the landing page is rendered by docs-site"),
        ("M\tdocs/parity.md", True, "the ledger is docs; the job that reads it is ungated"),
        ("M\tdocs/generated/policy-inventory.json", True, "same, and the gates job reads it"),
        ("D\tdocs/55-new.md", False, "deletion can break a link check_docs_links asserts"),
